          ...
          <tr>
            ...
            <td>
              <button
                class="action-btn"
                type="button"
                data-open-planning="${item.bloco_consolidado_id}"
                onclick="window.openPlanningModal('${item.bloco_consolidado_id}')"
              >
                Editar
              </button>
            </td>
...
function openPlanningModal(blockId) {
  const record = state.data.blocos.find(i => String(i.bloco_consolidado_id) === String(blockId));
  if (!record) {
    toast('Não foi possível abrir este bloco para edição.', true);
    return;
  }
...
function closePlanningModal() {
  ...
  el.planningModal.style.display = 'none';
