...
function renderSummary() {
  ...
  const aprovados = data.filter(i => (i.status_novo || i.status_atual) === 'APROVADO').length;
  const revisao = data.filter(i => i.precisa_revisao_planejamento).length;
  ...
  el.summaryCards.innerHTML = `
    ...
    <article class="summary-card">
      ...
      <small>Mudaram na última importação</small>
    </article>
    <article class="summary-card">
      ...
      <tbody>
        ${rows.map(item => `
          <tr>
            ...
            <td>${statusChip(item.status_novo || item.status_atual)}</td>
            <td>${statusChip(item.situacao_do_plano || 'SEM PLANO')}</td>
            <td>${formatNumber(item.total_agentes_empenhados || 0)}</td>
            <td>${item.precisa_revisao_planejamento ? '<span class="revision-flag">Precisa revisar</span>' : 'OK'}</td>
            <td>
